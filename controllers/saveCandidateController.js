...
const createSaveCandidateController = async (req, res) => {
  ...
  const getSaveCandidatesByCompanyIdController = async (req, res) => {
    try {
      ...
      const saveCandidate = await saveCandidateModel
        .find({ company: companyId })
        .sort({ createdAt: -1 })
        .skip((page - 1) * limit)
        .limit(Number(limit));
  
      const totalSaveCandidate = await saveCandidateModel.countDocuments({ company: companyId });
  
      if (!saveCandidate || saveCandidate.length === 0) {
        return res.status(404).send({
          success: false,
...
module.exports.getSaveCandidatesByCompanyIdController = getSaveCandidatesByCompanyIdController;
